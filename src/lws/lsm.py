...
def detect_lsm_state_dir(explicit: str | Path | None = None) -> Path | None:
    ...
    for name in ("LWS_LSM_STATE_DIR", "LOCAL_SHELL_MCP_STATE_DIR"):
    ...
    if os.name == "nt":
        ...
        if program_data:
            candidates.append(Path(program_data) / "LocalShellMCP-Hardened" / "control" / "state")
...
class FileLsmTelemetry:
    ...
    web chat message lifecycle is broken.
    """
    ...
    def _effective_job(row: dict[str, Any]) -> dict[str, Any]:
        ...
        membership). LWS only adopts a terminal attempt payload because that is a
