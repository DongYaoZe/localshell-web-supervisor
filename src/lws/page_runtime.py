...
PROBE_SLOT_SOURCE = "windows_uia_lws_probe"
...
def tagged_probe_url(conversation_url: str, *, slot_id: str, owner_token: str) -> str:
    ...
    fragment = f"lws-probe={slot_id}:{owner_token}"
...
def plan_probe_slot(
    ...
    *,
    ...
    checks: dict[str, bool] = {
        ...
        "target_is_web_conversation": _conversation_identity(worker.conversation_url) is not None,
    }
    if not all(checks.values()):
        return ProbeSlotPlan(
            ...
            reason="probe slots are only for parked workers with an existing web chat conversation URL",
            checks=checks,
        )
    ...
    if existing is None:
        return ProbeSlotPlan(
            ...
            reason="no probe slot is bound; open exactly one tagged LWS probe window",
            checks=checks,
    ...
    if not all(checks.values()):
        return ProbeSlotPlan(
            ...
            mutation_required=False,
            reason=(
                "existing probe slot is stale, ambiguous, or no longer proves LWS ownership; "
    ...
    if same_target:
        return ProbeSlotPlan(
            ...
            mutation_required=False,
            reason="fresh LWS-owned probe slot already targets this parked worker",
            checks=checks,
            owner_token=existing.owner_token,
        )
    ...
    return ProbeSlotPlan(
        ...
        mutation_required=True,
        reason="rotate the single LWS probe slot by exact-close-before-open",
        checks=checks,
        owner_token=uuid.uuid4().hex,
    )
...
def probe_close_operation(
    ...
    *,
    ...
    """Create an ARMED durable CLOSE operation for one exact LWS-owned probe slot."""
    ...
    if not slot_owns_actual_url(existing):
        raise ValueError("probe CLOSE requires an exact LWS ownership tag")
    ts = time.time() if now is None else float(now)
    return ProbeMutationOperation(
        operation_id=operation_id or f"probeop_{uuid.uuid4().hex}",
        nonce=nonce or uuid.uuid4().hex,
...
def observe_probe_slot(
    ...
    *,
    ...
    """Observe a tagged LWS-owned probe window and attribute it to its parked worker."""
    if worker.status != WorkerStatus.PARKED:
    ...
    if not slot_owns_actual_url(slot):
        raise UiaProbeUnavailable("probe slot no longer proves LWS URL ownership")
...
$expected=[string]$env:LWS_EXPECTED_ACTUAL_URL
...
$expected=[string]$env:LWS_EXPECTED_ACTUAL_URL; $expectedExe=[string]$env:LWS_EXPECTED_CHROME_EXE
...
if(-not $w){$obj=[pscustomobject]@{closed=$false;absent=$true;ambiguous=$false;detail='slot window already absent'}}
  ...
  if([string]$w.Current.ClassName -ne 'Chrome_WidgetWin_1' -or $browserPid -ne $pidExpected){$obj=[pscustomobject]@{closed=$false;absent=$false;ambiguous=$true;detail='HWND no longer matches bound Chrome identity'}}
    ...
    if(-not $proc -or -not ([string]$proc.Path).Equals($expectedExe,[StringComparison]::OrdinalIgnoreCase) -or -not $address -or $address.TrimEnd('/') -ne $expected.TrimEnd('/')){$obj=[pscustomobject]@{closed=$false;absent=$false;ambiguous=$true;detail='slot executable or ownership URL changed'}}
      ...
      try{$wp=$w.GetCurrentPattern([System.Windows.Automation.WindowPattern]::Pattern);$wp.Close();$obj=[pscustomobject]@{closed=$true;absent=$false;ambiguous=$false;detail='exact LWS probe window close requested'}}catch{$obj=[pscustomobject]@{closed=$false;absent=$false;ambiguous=$true;detail='exact window could not be closed safely'}}
    }
  }
}
...
class ChromeUiaProbeWindowTransport:
    ...
    LWS slot's exact HWND, PID, executable, and ownership-tagged URL. Rotation closes the old
    ...
    def _find(self, actual_url: str) -> dict[str, Any]:
        env = os.environ.copy()
        env["LWS_EXPECTED_ACTUAL_URL"] = actual_url
    ...
    def _close(self, slot: ProbeWindowSlotBinding) -> dict[str, Any]:
        ...
        env.update(
            {
                "LWS_EXPECTED_HWND": str(slot.window_handle),
                "LWS_EXPECTED_BROWSER_PID": str(slot.browser_pid),
                "LWS_EXPECTED_ACTUAL_URL": slot.actual_url,
                "LWS_EXPECTED_CHROME_EXE": slot.chrome_executable,
            }
        )
        return _run_ps(
            _CLOSE_SCRIPT,
        ...
        while time.time() < deadline:
            ...
            if len(matches) == 1:
                ...
                return ProbeSlotExecution(
                    True,
                    True,
                    "single tagged LWS probe window opened",
                    ProbeWindowSlotBinding(
    ...
    def execute(
        ...
        *,
        ...
        if plan.action == ProbeSlotAction.REUSE:
            ...
            return ProbeSlotExecution(False, False, "existing LWS probe window reused", refreshed)
