...
class TimeoutRecoveryPolicy:
    """Narrow opt-in gate for resident recovery of explicit web chat delivery errors.
...
def error_shadowed_by_active_generation(
    ...
    *,
    ...
    web chat can leave an old delivery-timeout banner visible after the user has manually
...
def acknowledged_state_is_unchanged(
    ...
    *,
    ...
    Old web chat error banners can remain visible after a successful continuation. The
...
def gate_timeout_dispatch_plan(
    ...
    *,
    ...
    if not checks["explicit_recoverable_delivery_error"]:
        blockers.append("no explicit recoverable web chat delivery error is present")
