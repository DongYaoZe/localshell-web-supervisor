...
try {
    ...
    if (-not [int64]::TryParse($env:LWS_EXPECTED_HWND, [ref]$expectedHwnd) -or $expectedHwnd -le 0) {
        Emit $false $false $phase 'an exact positive HWND is required'
        exit 0
    }
    $expectedPid = [int]0
    if (-not [int]::TryParse($env:LWS_EXPECTED_BROWSER_PID, [ref]$expectedPid) -or $expectedPid -le 0) {
    ...
    if (-not $doc) {
        Emit $false $false $phase 'web chat document is not present'
        exit 0
    }
    ...
    # web chat's application state can lag the accessibility ValuePattern update. Poll for a
...
$expectedExe = [string]$env:LWS_EXPECTED_CHROME_EXE
$expectedHwnd = [int64]0
if ($env:LWS_EXPECTED_HWND) { [void][int64]::TryParse($env:LWS_EXPECTED_HWND, [ref]$expectedHwnd) }
$expectedPid = [int]0
if ($env:LWS_EXPECTED_BROWSER_PID) { [void][int]::TryParse($env:LWS_EXPECTED_BROWSER_PID, [ref]$expectedPid) }
$nonce = [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String($env:LWS_ACK_NONCE_B64))
...
$doc = $window.FindFirst([System.Windows.Automation.TreeScope]::Descendants, $docCond)
if (-not $doc) { Emit-Error 'web chat document is not present'; exit 0 }
$all = $doc.FindAll(
    [System.Windows.Automation.TreeScope]::Descendants,
    [System.Windows.Automation.Condition]::TrueCondition
)
...
class ChromeUiaActionTransport:
    """Gated exact-window web chat sender for an already-ARMED action.

    The transport remains disabled by default. LWS exposes only an explicit one-shot
    ...
    def submit(self, intent: ActionIntent) -> TransportSubmission:
        ...
        env = os.environ.copy()
        env["LWS_EXPECTED_URL"] = self.conversation_url
        env["LWS_EXPECTED_HWND"] = str(self.expected_hwnd)
        env["LWS_EXPECTED_BROWSER_PID"] = str(self.expected_browser_pid)
        env["LWS_EXPECTED_CHROME_EXE"] = self.chrome_executable
        env["LWS_PROMPT_B64"] = base64.b64encode(intent.prompt.encode("utf-8")).decode("ascii")
...
class ChromeUiaAckObserver:
    ...
    def observe(
        ...
        *,
        ...
        env["LWS_EXPECTED_CHROME_EXE"] = self.chrome_executable
        env["LWS_ACK_NONCE_B64"] = base64.b64encode(expected_nonce.encode("utf-8")).decode("ascii")
        if expected_hwnd is not None:
            env["LWS_EXPECTED_HWND"] = str(int(expected_hwnd))
        if expected_browser_pid is not None:
            env["LWS_EXPECTED_BROWSER_PID"] = str(int(expected_browser_pid))
        result = _run_powershell_json(
            _POWERSHELL_ACK,
            powershell=self.powershell,
            timeout_s=self.timeout_s,
