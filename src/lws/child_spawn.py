...
def bind_child_spawn_window(
    ...
    *,
    ...
    if observed_url.rstrip("/") != attempt.tagged_project_url.rstrip("/"):
        raise ChildSpawnBlocked(["observed project window URL does not match the LWS ownership tag"])
    return registry.update_child_spawn_attempt(
        attempt_id,
...
def execute_child_spawn_prompt(
    ...
    *,
    ...
    if transition.url and web_conversation_project_id(transition.url) == submitted.project_id:
