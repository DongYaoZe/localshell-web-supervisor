...
`localshell-web-supervisor` is reliability infrastructure for work the user already performs through web chat and Local Shell MCP. Its purpose is to reduce lost task state, duplicate side effects, browser memory pressure, and unnecessary human polling. It is not a mechanism for bypassing authentication, platform safeguards, rate limits, or access controls.

## Default operating mode

LWS is **observe-first and fail-closed**.
...
- Treat a web chat conversation as a replaceable worker lease, never as the durable task identity.
...
- a dedicated LWS-controlled Playwright/browser profile created through ordinary user authentication;
- bounded DOM/network lifecycle metadata needed to determine liveness and silence.

For read-only Windows UI Automation, LWS transiently reads accessibility text only to derive
...
For LSM high-level browser snapshots, LWS does not persist the recent error/network entry
...
- never infer idempotence from the web chat UI;
...
LWS exposes an explicit one-shot current-worker executor. It requires exact task
...
After a crash, only one exact LWS-owned target may be adopted. Old+new present, multiple
...
- Modify only the LWS repository unless another path is explicitly authorized.
- Do not upgrade or patch Local Shell MCP as part of LWS experiments.
...
- Production LWS hosting should be independent of a web chat conversation and should use its own singleton watchdog lease.
...
`lws doctor` is intentionally diagnostic-only. It may read registry/LSM schema, local
...
When LWS asks another model or worker to inspect/recover a task, the instruction should state the safety boundary explicitly rather than relying on implication:

> This is a reliability/recovery task for a user-authorized web chat + Local Shell MCP workflow. Work read-only first. Do not bypass authentication or platform controls; do not access or move cookies, tokens, passwords, or private API credentials; do not call or reconstruct private web chat endpoints. Do not click/type in unrelated browser tabs. Before any action with side effects, reconcile the durable Local Shell session/plan/jobs and actual workspace/Git state. The previous message may have failed after side effects already completed, so never blindly replay the prior operation. If evidence is ambiguous, stop at a recommendation or request human review rather than guessing.
