...
This note records the empirical Local Shell MCP facts used to bootstrap LWS. It is intentionally separate from the architectural contract because these details can change when LSM is upgraded.
...
- hardened deployment state root: `%PROGRAMDATA%\\LocalShellMCP-Hardened\\control\\state`
- durable session schema version: `1`
- durable jobs-store schema version: `2`

The path above is an observed deployment detail, not a portable protocol. LWS supports an explicit state-directory override and environment-based discovery.
...
A resume with `takeover=true` creates a new agent run and supersedes the previous active run. LSM refuses takeover while tool calls are still in flight. LWS therefore must not invent a parallel takeover mechanism or edit session JSON directly.
...
This is strong reconciliation evidence: an apparently idle web chat UI cannot override a live durable tool lease.
...
LWS should observe and coordinate with this mechanism, not duplicate it. Its missing layer is web chat worker/message lifecycle health and cross-worker task continuity.
...
LWS V0 uses only facts safe to infer out of process:
...
- absence of a terminal status file is **not** treated as proof that a job died, because LWS does not own LSM's in-memory shell registry.
...
During the LWS bootstrap, a resident `python -m lws ... watch` process was launched twice
...
not guarantee descendant process-tree termination. LWS therefore treats LSM job status
...
It does not yet expose enough stream lifecycle timing to distinguish every web chat generation/delivery stall. V1 should therefore add timestamped network/CDP observability, while continuing to use DOM + durable LSM evidence as independent signals.

## Compatibility rule

The direct-file adapter is an implementation bridge for the inspected file backend, not a claim that LSM's JSON files are a stable public API. LWS gates known schema versions and fails closed on unknown versions. If a future LSM release changes these formats, update and test the adapter before allowing classification/recovery decisions.
